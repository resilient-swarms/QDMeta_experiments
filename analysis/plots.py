...
def make_table(f,stats,rowlabels,columnlabels, conditionalcolumnlabels=[],median=False,transpose=False):
    ...
    if columnlabels:
        ...
        for i in range(len(rowlabels)):
            ...
            stat = stats[i]
            if median:
                table_entry_median(f, stat, conditionalcolumnlabels[0][1])
            else:
                table_entry_meansd(f,stat,conditionalcolumnlabels[0][1])
            newline_latex(f)
...
def createBarplot(stats,x_values,colors,markers,xlabel,ylabel,ylim,save_filename,legend_labels,yscale="linear",
    ...
    while True:
        ...
        axes = fig.gca()
        axes.set_xlabel(xlabel, fontsize=45)
        axes.set_ylabel(ylabel, fontsize=45)
...
def createPlot(stats,x_values,colors,markers,xlabel,ylabel,ylim,save_filename,legend_labels,xlim=None,xscale="linear",yscale="linear",
               ...
               legend_cols=1,legend_fontsize=18,legend_indexes=[],additional_lines=[],index_x=[],
               xaxis_style="plain",y_err=[],force=False,fill_between=[],line_width=None,ms=None,
               ax=None, title=None,legend_setting="default"):
    if line_width is None:
        line_width=[5.0 for i in stats]
        #line_width[1]=8
    if ms is None:
    ...
    while True:
        ...
        ax.grid(True)
        if legend_setting=="separate":

            leg = ax.legend(lines, labels=legend_labels, ncol=2,
                                prop={'size': 14},
                                fancybox=True,bbox_to_anchor=(1.2, 1.2))
            fig.tight_layout()
            fig.savefig(save_filename)
            return
        elif legend_setting=="empty":
            print("will not make a legend")
        else:
