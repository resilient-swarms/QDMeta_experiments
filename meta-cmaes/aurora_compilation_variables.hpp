#ifndef AURORA_COMPILATION_HPP
#define AURORA_COMPILATION_HPP
...
namespace aurora
{
    ...
    {
        return (
            (get_algo() == algo::Algo::hand_coded_qd) || (get_algo() == algo::Algo::hand_coded_qd_no_sel) || (get_algo() == algo::Algo::hand_coded_taxons));
    }

    constexpr bool is_algo_taxons_based()
    {
        return (
            (get_algo() == algo::Algo::taxons) || (get_algo() == algo::Algo::taxo_s) || (get_algo() == algo::Algo::taxo_n) || (get_algo() == algo::Algo::hand_coded_taxons));
    }
    ...
    does_encode_images()
    {
        return (get_encoder_type() == aurora::EncoderType::cnn_ae) or (get_encoder_type() == aurora::EncoderType::strg_cnn);
    }
    ...
    inline constexpr bool get_has_fit()
    {
#ifdef HAS_FIT
        return true;
#else
    ...
    inline constexpr bool use_volume_adaptive_threshold()
    {
#ifdef VAT
        return true;
#else
    ...
    inline constexpr bool no_normalisation_bd_from_latent_space()
    {
#ifdef NO_NORMALISE
        return true;
#else
    ...
    inline constexpr bool use_elitism_in_taxons()
    {
#ifdef TAX_ELI
        return true;
#else
